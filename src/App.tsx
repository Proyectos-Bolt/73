...
function App({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  // Estado para tipo de tarifa (normal o especial)
  const [tarifaTipo, setTarifaTipo] = useState<'normal' | 'especial'>('normal');
  ...
  const getBasePrice = (tripType: TripType): number => {
    ...
    const baseFareFromType = tarifaTipo === 'especial' ? RATES.baseFareEspecial : RATES.baseFare;
  ...
  const calculateFare = useCallback((distanceKm: number, waitingMinutes: number, sorianaBonus: boolean = false, accumulatedStopsCost: number = costoAcumuladoParadas) => {
    ...
    if (servicioEspecialConfig.active) {
      ...
      // Usar el precio base según si es tarifa normal o especial
      const baseFareFromType = tarifaTipo === 'especial' ? RATES.baseFareEspecial : RATES.baseFare;
  ...
  }, [selectedTripType, selectedSubTrip, petConfig, servicioEspecialConfig, personasExtrasConfig, costoAcumuladoParadas, selectedSorianaZone, tarifaTipo]);
  ...
  const stopTrip = () => {
    ...
    setTripData({
      distance: 0,
      rawDistance: 0,
      cost: tarifaTipo === 'especial' ? RATES.baseFareEspecial : RATES.baseFare,
      waitingTime: 0,
      isRunning: false,
      isPaused: false
    });
      ...
      try {
        ...
        if (user) {
          ...
          if (docSnapshot.exists()) {
            ...
            const todayEarnings = ganancias.find((g: any) => g.fecha === today);
            setDailyEarnings(todayEarnings?.total || 0);
          }
        }
      } catch (error) {
                ...
                onClick={async () => {
                  try {
                    ...
                    if (user && dailyEarnings > 0) {
                      ...
                      let ganancias = [];
                      if (docSnapshot.exists()) {
                        ganancias = docSnapshot.data().ganancias || [];
                      }

                      const existingIndex = ganancias.findIndex((g: any) => g.fecha === today);
                      if (existingIndex === -1) {
                        ganancias.push({ fecha: today, total: 0 });
                      }
          ...
          <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
            <div className="bg-gradient-to-br from-gray-800 to-gray-900 border border-red-500 rounded-xl p-6 max-w-sm w-full shadow-2xl">
              ...
              <div className="space-y-3">
                ...
                <button
                  onClick={() => alert('Funcionalidad en desarrollo: Ver estadísticas generales')}
                  className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl transition duration-200 text-sm"
                >
                  Ver Estadísticas
                </button>
        ...
        <div className="bg-gray-800 p-4 rounded-xl shadow-lg mb-6 border border-gray-700">
          ...
          {showRates && (
            <div className="mt-3 border-t border-gray-700 pt-3 space-y-3 text-sm text-gray-300">
              <p className="font-bold text-white">Tarifa Base: ${tarifaTipo === 'especial' ? RATES.baseFareEspecial : RATES.baseFare} MXN ({tarifaTipo === 'especial' ? 'Especial' : 'Normal'})</p>
